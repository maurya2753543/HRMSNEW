...
export class CreateUpdateClientComponent implements OnInit {
  ...
  editorConfig = {
    placeholder: 'Enter about client...',
    height: '500px'
  };
  ...
  createClient() {
    ...
    this.dataService.saveData(this.context.client, formData).subscribe((res) => {
      if(res){
        this.snackBar.open(`"${res.client}" client created successfully.`, "Ok", {
          duration: 4000,
        });
        this.close(res);
      }
  ...
  updateClient() {
    ...
    let url = `${this.context.client}/${formData.id}`;
    this.dataService.updateData(url, formData).subscribe((res) => {
      this.snackBar.open(`"${formData.client}" client updated successfully.`, "Ok", {
        duration: 4000,
      });
      this.close(formData);
      this.loader.hide();
